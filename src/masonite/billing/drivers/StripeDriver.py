...
class StripeDriver(BaseDriver):
    def subscribe(self, plan, token, customer=None, **kwargs):
        ...
        try:
            subscription = self._create_subscription(customer, plan=plan, **kwargs)
            return subscription
    ...
    def cancel(self, plan_id, now=False):
        ...
        """
        subscription = stripe.Subscription.retrieve(plan_id, api_key=self.options.get("secret"))
    ...
    def charge(self, amount, **kwargs):
        ...
        charge = stripe.Charge.create(amount=amount, api_key=self.options.get("secret"), **kwargs)
    ...
    def swap(self, plan, new_plan, **kwargs):
        ...
        """
        subscription = stripe.Subscription.retrieve(plan, api_key=self.options.get("secret"))
    ...
    def plan(self, plan_id):
        ...
        product = stripe.Product.retrieve(
            subscription["plan"]["product"], api_key=self.options.get("secret")
        )
        return product["name"]
        ...
        if "coupon" in self._subscription_args:
            if type(self._subscription_args["coupon"]) == str:
                coupon = stripe.Coupon.retrieve(
                    self._subscription_args["coupon"], api_key=self.options.get("secret")
                )
    ...
    def _create_customer(self, description, token):
        ...
        Arguments:
            ...
            token {string} -- The Stripe token from the form submission (obtained with Stripe.js).
        Returns:
            stripe.Customer.create
        """
        return stripe.Customer.create(
            description=description, source=token, api_key=self.options.get("secret")
        )
    ...
    def _get_subscription(self, plan_id):
        ...
        """
        return stripe.Subscription.retrieve(plan_id, api_key=self.options.get("secret"))
